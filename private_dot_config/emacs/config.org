...
(calum/leader-def :infix "M"
  ...
  "" nil
  "w" 'writeroom-mode
  "c" 'calum/copy-current-kill-to-clipboard
...
(use-package meow
  ...
  (defun calum/meow-undo-in-selection ()
    "Like =meow-undo-in-selection= but allow for a fallback."
    (interactive)
  ...
  (defun calum/meow-setup ()
    ...
    (meow-normal-define-key
     ...
     '("s" . meow-kill)
     '("t" . meow-till-expand)
     '("u" . calum/meow-undo-in-selection)
     '("U" . undo-tree-redo)
     '("v" . meow-find-ref)
     '("V" . meow-pop-marker)
     '("w" . meow-mark-word)
  ...
  (defmacro meow-nav--mk-movement (fwd bkwd name doc)
    `(defun ,name (arg)
       ,doc
       (interactive "P")
       (if (equal 'forward meow-nav--direction)
           (funcall-interactively ,fwd arg)
         (funcall-interactively ,bkwd arg))))
  ...
  (meow-nav--mk-movement 'diff-hl-next-hunk 'diff-hl-previous-hunk meow-nav-move-hunk "Move by version control hunk")
  ...
  (defun calum/meow-setup-extra ()
    ...
    ;; Use u to undo only the changes in the selection, or just do
    ;; regular undo if there is no selection
    (add-to-list 'meow-selection-command-fallback '(calum/meow-undo-in-selection . meow-undo))
...
(org-level-8 ((nil :foreground "#E5E9F0" :weight bold :inherit 'default)))
...
(org-level-2 ((nil :inherit 'org-level-8 :height 1.2)))
(org-level-1 ((nil :inherit 'org-level-8 :height 1.3)))
...
(use-package tempel
  ...
  :general
  (calum/leader-def "M t" 'tempel-insert)
  :hook
...
| p/P        | meow-yank/nil                             | previous-line/execute-extended-command | nil/nil                                     | previous-line                       | project funcs            |                 | nil/nil     |
...
| f/F        | meow-find-expand/nil                      | forward-char/nil                       | forward-word/nil                            | forward-sexp                        | format buffer            |                 | -/nil       |
| g/G        | meow-cancel-selection/meow-grab           | keyboard-quit/nil                      | consult goto/nil                            | nil                                 |                          |                 | -/-         |
...
| z/Z        | meow-pop-selection/nil                    | suspend-frame/nil                      | zap-to-char/nil                             | nil                                 |                          |                 | -/nil       |
...
| $/4        | nil/meow-expand-4                         | nil/digit-argument                     | ispell-word/digit-argument                  | nil/digit-argument                  | /meow-digit-argument     |                 | nil/-       |
| %/5        | query-replace-map/meow-expand-5           | nil/digit-argument                     | query-replace/digit-argument                | query-replace-regexp/digit-argument | /meow-digit-argument     |                 | nil/-       |
