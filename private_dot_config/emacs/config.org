...
** Text editing
*** Devil mode
This gives a nice way to enter some of emacs' longer shortcuts which will work in any evil state and any buffer. To avoid getting too complicated getting this to play nice with evil or messing with the comma key in insert mode, I use an obscure key and place that in a convenient location on my keyboard.
