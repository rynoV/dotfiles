...
(use-package kkp
  ...
  (defun calum/set-C-i-tab ()
    (when (or (display-graphic-p) (kkp--this-terminal-has-active-kkp-p))
...
(use-package general
  ...
  ;; Note: when using :infix it is necessary to unbind the infix key using ~"" nil~ to avoid errors about "key sequence starts with non-prefix key"
...
(defun wsl-copy (start end)
  (interactive "r")
  ;; fish_clipboard_copy works (at least with clip.exe installed), but it will fail with a message "stdout is not a terminal" that we can ignore
  (let ((inhibit-message t))
    (shell-command-on-region start end "fish_clipboard_copy" )
    )
  (deactivate-mark))
...
(defun wsl-paste ()
  (interactive)
  (let ((clipboard
     (shell-command-to-string "fish_clipboard_paste")))
    (setq clipboard (replace-regexp-in-string "\r" "" clipboard)) ; Remove Windows ^M characters
    (setq clipboard (substring clipboard 0 -1)) ; Remove newline
    (insert clipboard)))
#+end_src
...
;; Map home and end to get consistent behaviour an macos
("<home>" 'move-beginning-of-line)
("<end>" 'move-end-of-line)
("S-<delete>" (general-key "<cut>"))
("C-<insert>" :predicate '(lambda () (not calum/is-wsl)) (general-key "<copy>"))
("C-<insert>" :predicate '(lambda () calum/is-wsl) 'wsl-copy)
...
Needs ~copilot-install-server~ and ~copilot-login~
...
(use-package copilot
  ...
  :custom
  ; This package prints a warning if you don't specify each mode's
  ; indentation in copilot-indentation-alist, but it uses a default so
  ; the warning can be ignored
  (copilot-indent-offset-warning-disable t)
  ; Increase max file size for copilot, for my large org files
...
(use-package ghostel
  ...
  (calum/leader-def
    ...
    "t" '(ghostel-other :which-key "New or next terminal"))
  :custom
  ; Always send escape key to the terminal, use C-\ C-n to go to normal mode
  (evil-ghostel-escape 'terminal))
(use-package evil-ghostel
  :after (ghostel evil)
  :hook (ghostel-mode . evil-ghostel-mode))
;; These use :straight nil because they are packaged with ghostel
...
(use-package magit
  ...
  (setq forge-add-default-bindings t
        ;; If enabling syntax highlighting with magit-diff-fontify-hunk, this variable needs to be set here I think
  ...
  (:keymaps 'magit-mode-map
            "SPC" nil
            "SPC" (general-key calum/leader-non-normal)
            ":" nil
            ":" 'evil-ex
            "C-c SPC" 'magit-diff-show-or-scroll-up
            "C-c RET" 'calum/magit-diff-visit-file-nvim
            ;; Shift-tab
  ...
  (defun calum/set-frame-nvim (nvim)
    "Set the NVIM socket used by commands launched from the current frame."
    (interactive "sNVIM socket: ")
    (set-frame-parameter nil 'calum-nvim nvim))
  (defun calum/set-frame-magit-mode (&optional enabled)
    "Set whether Magit uses full-frame status buffers in the current frame."
    (interactive "P")
    (set-frame-parameter nil 'calum-magit-mode enabled))
  (defun calum/magit-display-buffer-function (buffer)
    "Display BUFFER according to the current frame's Magit setting."
    (if (frame-parameter nil 'calum-magit-mode)
        (magit-display-buffer-fullframe-status-v1 buffer)
      (magit-display-buffer-traditional buffer)))
  (defun calum/nvim-frame (nvim)
    "Return the live frame associated with the Neovim socket NVIM."
    (seq-find
     (lambda (frame)
       (and (frame-live-p frame)
            (equal (frame-parameter frame 'calum-nvim) nvim)))
     (frame-list)))
  (defun calum/with-nvim-frame (nvim function)
    "Call FUNCTION in the frame associated with the Neovim socket NVIM."
    (let ((frame (calum/nvim-frame nvim)))
      (unless frame
        (user-error "No Emacs frame is associated with Neovim socket %s" nvim))
      (with-selected-frame frame
        (funcall function))))
  (defun calum/nvim-magit-status (nvim directory)
    "Show Magit status for DIRECTORY in the Neovim-associated frame."
    (calum/with-nvim-frame
     nvim
     (lambda ()
  ...
  (defun calum/nvim-magit-file-position (nvim directory file line column)
    "Show FILE's Magit status position in the Neovim-associated frame."
    (calum/with-nvim-frame
     nvim
     (lambda ()
       (let* ((file (expand-file-name file))
              (directory (file-name-as-directory (expand-file-name directory)))
              (toplevel (or (magit-toplevel (file-name-directory file))
                            (magit-toplevel directory))))
         (unless (file-regular-p file)
           (user-error "%s is not a regular file" file))
         (unless toplevel
           (user-error "%s is not inside a Git worktree" file))
         (with-current-buffer (find-file-noselect file)
           (save-excursion
             ...
             (let ((magit-status-goto-file-position t))
               (magit-status toplevel))))))))
  (defun calum/nvim-magit-refresh (nvim)
    "Refresh the visible Magit status buffer in the Neovim-associated frame."
    (calum/with-nvim-frame
     nvim
     (lambda ()
       (when-let ((buffer
                   (seq-some
                    (lambda (window)
                      (let ((candidate (window-buffer window)))
         ...
         (with-current-buffer buffer
           (magit-refresh))))))
  (defun calum/magit-diff-visit-file-nvim ()
    "Visit the diff at point with Neovim.

This only works when there is a running neovim instance and the 'calum-nvim' frame parameter is set to the socket for that instance.

When starting a new emacs frame from within the neovim terminal, you can start it like this to enable magit->nvim integration:
'emacsclient -a '' -c -nw -F \"((calum-nvim . \"$NVIM\"))\"'

Normally this goes to the worktree file for added/context lines in
staged/unstaged sections. On a deleted line or in other diff contexts,
it opens the corresponding blob buffer so the relevant diff content
remains visible. Use with a prefix argument to switch to the opposite
destination.
"
    (interactive)
    (unless (frame-parameter nil 'calum-nvim)
      (user-error "The current frame has no calum-nvim parameter"))
    (let ((goto-file
           (let ((normal-goto-file
                  (and (memq (magit-diff--dwim) '(staged unstaged))
                       (not (magit-diff-on-removed-line-p)))))
             (if current-prefix-arg
                 (not normal-goto-file)
               normal-goto-file))))
      (pcase-let ((`(,buffer ,position)
                   (magit-diff-visit-file--noselect goto-file)))
        (unless (buffer-live-p buffer)
          (user-error "Magit could not create a file buffer"))
        (if-let ((file (buffer-file-name buffer)))
          (let (line column)
            ...
            (let ((process-environment (copy-sequence process-environment))
              ...
              (setenv "NVIM" (frame-parameter nil 'calum-nvim))
              (start-process
               (format "magit-diff-nvim-%s" (buffer-name buffer))
               nil
               (expand-file-name "~/scripts/nvim-cmd.sh")
               (format "+call cursor(%d, %d)" line column)
               file)))
        (with-current-buffer buffer
          (save-restriction
            (widen)
            ...
            (let* ((name (replace-regexp-in-string
                          "'" "''" (buffer-name buffer) t t))
                   (filename (replace-regexp-in-string
                              "'" "''" (magit-buffer-file-name) t t))
                   (set-name-command
                    (format "let g:flatten_stdin_name = '%s'" name))
                   (set-filename-command
                    (format "let g:flatten_stdin_filename = '%s'" filename))
                   (cursor-command
                    (format "+call cursor(%d, %d)"
                            (line-number-at-pos)
                            (1+ (current-column))))
              ...
              (call-process-region
               (point-min) (point-max)
               (expand-file-name "~/scripts/nvim-cmd.sh")
               nil nil nil "--cmd" set-name-command
               "--cmd" set-filename-command cursor-command "-"))))))))
  :custom
  (magit-section-show-context-menu-for-emacs<28 t)
  :config
  ;; Set the `calum-magit-mode' frame parameter to use full-frame status
  ;; buffers in a frame.
  (setq magit-display-buffer-function #'calum/magit-display-buffer-function)
...
(use-package evil
  ...
  (calum/leader-def :infix "w"
    "" '("Windows" . evil-window-map))
  (:states '(insert normal visual emacs)
            "C-\\" nil
            "C-\\ C-n" 'evil-force-normal-state)
  :config
  (evil-mode)
