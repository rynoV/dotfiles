...
Normally this goes to the worktree file for added/context lines in staged/unstaged sections. On a deleted line, it opens the corresponding blob buffer so the deleted code remains visible. Use with a prefix argument to force blob visiting everywhere.
"
    (interactive)
